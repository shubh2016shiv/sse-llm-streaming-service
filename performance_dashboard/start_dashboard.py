#!/usr/bin/env python3
"""
Performance Dashboard - Professional Python Startup Script

This script provides a robust, cross-platform way to start the Performance Dashboard
with proper prerequisite checks and user interaction.
...
Usage:
    python start_dashboard.py [--auto-start] [--skip-build] [--health-timeout SECONDS]
...
class DashboardStarter:
    """Professional dashboard starter with comprehensive error handling."""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Start the Performance Dashboard with prerequisite checks",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python start_dashboard.py                    # Interactive mode (default)
  python start_dashboard.py --auto-start      # Auto-start backend if needed
  python start_dashboard.py --skip-build      # Skip build if image exists
  python start_dashboard.py --health-timeout 5  # 5-second health check timeout
        """
    )
    ...
    parser.add_argument(
        "--auto-start",
        action="store_true",
        help="Automatically start backend if not running (like bash script)"
    )

    parser.add_argument(
        "--skip-build",
        action="store_true",
        help="Skip Docker image build step (faster if image already exists)"
    )
